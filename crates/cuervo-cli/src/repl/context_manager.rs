...
use cuervo_context::assembler::{assemble_context, chunks_to_system_prompt};
use cuervo_context::pipeline::{ContextPipeline, ContextPipelineConfig};
use cuervo_core::traits::{ContextQuery, ContextSource};
...
mod tests {
    ...
    impl ContextSource for MockSource {
        ...
        async fn gather(
            &self,
            _query: &ContextQuery,
        ) -> cuervo_core::error::Result<Vec<ContextChunk>> {
            Ok(vec![ContextChunk {
    ...
    fn add_message_delegates() {
        ...
        let msg = ChatMessage {
            ...
            content: cuervo_core::types::MessageContent::Text("hello".into()),
        };
        mgr.add_message(msg);
    ...
    fn scoped_for_sub_agent_limits_history() {
        ...
        for i in 0..5 {
            mgr.add_message(ChatMessage {
                role: cuervo_core::types::Role::User,
                content: cuervo_core::types::MessageContent::Text(format!("msg {i}")),
                });
        }
