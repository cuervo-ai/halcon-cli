...
mod tests {
    ...
    fn extract_paths_from_text() {
        let text = "Let me read src/main.rs and check crates/cuervo-core/src/lib.rs";
        let paths = extract_file_paths(text);
        assert_eq!(paths.len(), 2);
        assert!(paths.contains(&"src/main.rs".to_string()));
        assert!(paths.contains(&"crates/cuervo-core/src/lib.rs".to_string()));
    }
    ...
    fn predict_file_read_from_text() {
        let messages = vec![
            text_msg(Role::User, "Please check crates/cuervo-cli/src/main.rs for the entry point"),
        ];
        let predictions = predict_tool_calls(&messages);
    ...
    async fn speculate_with_file_paths() {
        ...
        let config = cuervo_core::types::ToolsConfig {
