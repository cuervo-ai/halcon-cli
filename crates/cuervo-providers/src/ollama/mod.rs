...
impl OllamaProvider {
    ...
    async fn send_and_collect(
        ...
        let response = self
            ...
            .map_err(|e| {
                if e.is_connect() {
                    CuervoError::ConnectionError {
                        provider: "ollama".into(),
                        message: format!("Cannot connect to Ollama at {}: {e}", self.base_url),
                    }
                } else if e.is_timeout() {
                    CuervoError::RequestTimeout {
                        provider: "ollama".into(),
                        timeout_secs: 300,
                    }
                } else {
                    CuervoError::ApiError {
        ...
        let body = response.text().await.map_err(|e| {
            CuervoError::StreamError(format!("read error: {e}"))
        })?;
    ...
    fn parse_ndjson_line(line: &str) -> Option<Result<ModelChunk>> {
        ...
        let response: OllamaChatResponse = match serde_json::from_str(line) {
            ...
            Err(e) => {
                ...
                return Some(Err(CuervoError::StreamError(format!(
                    "invalid NDJSON: {e}"
                ))));
            }
        };
                    ...
                    let bytes = match chunk_result {
                        ...
                        Err(e) => {
                            return std::future::ready(Some(vec![Err(
                                CuervoError::StreamError(format!("stream read error: {e}")),
                            )]));
                        }
                    };
...
mod tests {
    use super::*;
    use cuervo_core::types::{ChatMessage, ContentBlock, MessageContent, Role, ToolDefinition};
