...
impl AnthropicProvider {
    ...
    fn build_api_request(request: &ModelRequest) -> ApiRequest {
        let messages: Vec<ApiMessage> = request
            .messages
            .iter()
            .filter(|m| m.role != cuervo_core::types::Role::System)
            .map(|m| ApiMessage {
                role: match m.role {
                    cuervo_core::types::Role::User => "user".into(),
                    cuervo_core::types::Role::Assistant => "assistant".into(),
                    cuervo_core::types::Role::System => unreachable!(),
                },
    ...
    match &msg.content {
        ...
        MessageContent::Blocks(blocks) => {
            let api_blocks: Vec<ApiContentBlock> = blocks
                .iter()
                .map(|b| match b {
                    cuervo_core::types::ContentBlock::Text { text } => {
                        ApiContentBlock::Text { text: text.clone() }
                    }
                    cuervo_core::types::ContentBlock::ToolUse { id, name, input } => {
    ...
    match &msg.content {
        ...
        MessageContent::Blocks(blocks) => blocks
            ...
            .filter_map(|b| match b {
                cuervo_core::types::ContentBlock::Text { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<Vec<_>>()
            .join("\n"),
...
impl ModelProvider for AnthropicProvider {
    ...
    async fn invoke(
        ...
        let body: bytes::Bytes = serde_json::to_string(&api_request)
            .map_err(|e| CuervoError::ApiError {
        ...
        for attempt in 0..=max_retries {
            ...
            let response = match tokio::time::timeout(request_timeout, send_fut).await {
                ...
                Ok(Err(e)) => {
                    ...
                    if e.is_timeout() {
                        let err = CuervoError::RequestTimeout {
                            provider: "anthropic".into(),
                    ...
                    if e.is_connect() {
                        let err = CuervoError::ConnectionError {
                            provider: "anthropic".into(),
                            message: format!("{e}"),
                        };
                        if attempt < max_retries {
            ...
            if status_code == 401 {
                ...
                return Err(CuervoError::AuthFailed(msg));
            }
            ...
            if http::is_retryable_status(status_code) && attempt < max_retries {
                ...
                last_error = Some(CuervoError::ApiError {
        ...
        Err(last_error.unwrap_or_else(|| CuervoError::ApiError {
...
mod tests {
    ...
    fn build_api_request_basic() {
        let req = ModelRequest {
            ...
            messages: vec![ChatMessage {
                role: cuervo_core::types::Role::User,
    ...
    fn build_api_request_filters_system_messages() {
        let req = ModelRequest {
            ...
            messages: vec![
                ChatMessage {
                    role: cuervo_core::types::Role::System,
    ...
    fn message_to_text_from_blocks() {
        let msg = ChatMessage {
            role: cuervo_core::types::Role::Assistant,
    ...
    fn build_api_request_forwards_tools() {
        let req = ModelRequest {
            ...
            tools: vec![cuervo_core::types::ToolDefinition {
                name: "file_read".into(),
