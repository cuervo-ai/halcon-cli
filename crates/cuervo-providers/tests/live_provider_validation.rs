...
//!
//!   OPENAI_API_KEY=... DEEPSEEK_API_KEY=... GEMINI_API_KEY=... cargo test -p cuervo-providers --test live_provider_validation
//!
//! Or load from .env:
...
use cuervo_core::types::{ChatMessage, HttpConfig, MessageContent, ModelChunk, ModelRequest, Role};
...
async fn ollama_connectivity() {
    use cuervo_providers::OllamaProvider;
...
async fn openai_gpt4o_mini_connectivity() {
    use cuervo_providers::OpenAIProvider;
...
async fn deepseek_chat_connectivity() {
    use cuervo_providers::DeepSeekProvider;
...
async fn gemini_flash_connectivity() {
    use cuervo_providers::GeminiProvider;
...
async fn cross_provider_same_question_all_respond() {
    use cuervo_providers::{DeepSeekProvider, GeminiProvider, OllamaProvider, OpenAIProvider};
...
fn model_selector_with_real_model_info() {
    use cuervo_core::types::ModelSelectionConfig;
    use cuervo_providers::{EchoProvider, ProviderRegistry};
