...
use cuervo_core::types::validation::strip_orphaned_tool_results;
...
impl ContextPipeline {
    ...
    fn total_original_tokens_l4_adjust(&mut self, loaded: &ColdArchive) {
        ...
        if loaded_tokens > 0 {
            let _ = self.accountant.allocate(Tier::L4Cold, loaded_tokens);
        }
    }
...
mod tests {
    ...
    fn refresh_instructions_no_change_returns_none() {
        let dir = tempfile::TempDir::new().unwrap();
        std::fs::write(dir.path().join("CUERVO.md"), "# Instructions\nUse Rust.").unwrap();
    ...
    fn refresh_instructions_new_file_detected() {
        let dir = tempfile::TempDir::new().unwrap();
        let mut pipeline = ContextPipeline::new(&default_config());
        // Initialize with no CUERVO.md.
        pipeline.initialize("system prompt", dir.path());

        // Create a new CUERVO.md — should be detected.
        std::fs::write(dir.path().join("CUERVO.md"), "new instructions").unwrap();
    ...
    fn pair_eviction_keeps_tool_pair_together() {
        ...
        let violations = cuervo_core::types::validation::validate_message_sequence(&msgs, false);
        let orphans: Vec<_> = violations
            .iter()
            .filter(|v| matches!(v, cuervo_core::types::validation::ProtocolViolation::OrphanedToolResult { .. }))
            .collect();
        assert!(
            orphans.is_empty(),
